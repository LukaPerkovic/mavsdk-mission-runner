...
int main(int argc, char **argv)
{
    ...
    MissionConfig cfg =
        load_mission_config("config/mission_config_issue_sim.yaml");
    ...
    if (argc > 2)
    {
        ...
        {
            std::cerr << "Unrecognized position argument on index [2]"
                      << nav_mode_arg << '\n';
            return 1;
        }
    ...
    if (cfg.navigation_mode == "relative")
    {
        ...
        Coords destination = startPosition;
        for (const auto &wp : cfg.relative_waypoints)
        {
            destination = getDestinationCoords(destination, wp.azimuth_deg,
                                               wp.distance_m);
